...
- (c) **キーボードショートカット導入の可否と割当**: Issue では `e` / `a` ショートカット案と既存
  `e` キーとの衝突回避が論点として挙げられている。本リポジトリの `static/app.js` には既に
  `/`（検索フォーカス）/ `n`（quick-add 遷移）/ `?`（ショートカット overlay トグル）/ `j` / `k`
  （カード間ナビゲーション）/ `o` / `Enter`（フォーカス中カードのリンクを開く）/ `e`（詳細
  ページの `button.edit-tags` クリック）のグローバル `keydown` ハンドラが存在しており、`e` は
  詳細ページの edit-tags で既に予約されている。Chrome 拡張・MCP 側で `e` キーが暗黙に予約
  されている用途の有無、および本 Issue でショートカットを正式機能として含めるか optional と
  するか、含める場合の割当（`e` / `a` / その他、上記既存予約と衝突しないもの）を決定する必要
  がある。決定によって Requirement 2 にショートカット用の AC を追加する
